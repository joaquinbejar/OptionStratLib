...
impl DeltaNeutrality for BearPutSpread {
    fn calculate_net_delta(&self) -> DeltaInfo {
        let long_put_delta = self.long_put.option.delta().unwrap();
        let short_put_delta = self.short_put.option.delta().unwrap();
...
mod tests_delta_size {
    ...
    fn create_test_reducing_adjustments() {
        ...
        assert_decimal_eq!(delta, -size, DELTA_THRESHOLD);
        assert_decimal_eq!(
            strategy.calculate_net_delta().net_delta + delta,
            Decimal::ZERO,
            DELTA_THRESHOLD
        );
    }
    ...
    fn test_adjust_nonexistent_position() {
        ...
        match result.unwrap_err().downcast_ref::<PositionError>() {
            Some(PositionError::ValidationError(
                ...
                assert_eq!(
                    reason,
                    "Call is not valid for BearPutSpread"
                );
            }
