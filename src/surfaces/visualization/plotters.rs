...
/// Implementation of the `PlotBuilderExt` trait for `PlotBuilder<Vec<Surface>>`.
///
...
///
/// # Functionality
/// - **Surface Points Preparation**: It iterates over the curve data (`data`) and transforms
...
/// - **Surface Drawing**: Each curve is drawn using the `LineSeries` feature from `plotters`.
...
///   rendering, or plot saving, by propagating them as `SurfaceError` instances.
///
/// # Parameters
...
/// - Returns `Err(SurfaceError)` on failure, encapsulating the failure reason as a string.
///
/// # Dependencies
...
/// as `SurfaceError` with a `StdError` variant and a descriptive error message.
///
/// # Algorithm
/// 1. **Fetch Surface Points**: Convert the curves' `Point3D` instances to `(f64, f64)` tuples.
...
/// 6. **Draw Surfaces**: Iterate through the prepared curve points and draw each curve with a distinct color.
...
/// - The success of the plot rendering depends on valid and well-formed curve data (`Vec<Surface>`).
///
/// # Examples of Dependencies
/// - **Associated Traits**: Must be used with the `PlotBuilder` struct and a compatible `Vec<Surface>` data type.
...
/// - **`SurfaceError`**: Represents errors that can occur while saving the plot.
///
/// # Remarks
...
/// - The precision of `Point3D::x` and `Point3D::y` values is preserved by converting them from
///   `Decimal` to `f64` when plotting.
impl PlotBuilderExt<Vec<Surface>> for PlotBuilder<Vec<Surface>> {
    #[cfg(target_arch = "wasm32")]
    fn save(self, _path: impl AsRef<Path>) -> Result<(), SurfaceError> {
        // Do nothing in wasm
        Ok(())
    ...
    fn save(self, path: impl AsRef<Path>) -> Result<(), SurfaceError> {
        ...
        for (i, points) in all_curve_points.into_iter().enumerate() {
            let default_name = &format!("Surface {}", i + 1);
...
mod tests {
    ...
    fn test_multiple_curves_plot() {
        ...
        let curve1 = Surface::from_vector(points1);
        let curve2 = Surface::from_vector(points2);
