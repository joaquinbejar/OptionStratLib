...
- **`Simulate` and `SimulationStats` move to backtesting** (#595, multi-crate
  ...
  becomes `src/backtesting/stats.rs`; `simulation::Simulate`,
  `simulation::SimulationStats` and the prelude paths are re-exports marked
  `// facade-compat: backtest`, so no 0.21 path changes. The simulation layer
