...
fn create_test_chain_with_liquidity() -> OptionChain {
    let mut chain = OptionChain::new("TEST", pos!(450.0), "2024-12-31".to_string(), None, None);
...
fn create_chain_without_liquidity() -> OptionChain {
    let mut chain = OptionChain::new("NOLIQ", pos!(450.0), "2024-12-31".to_string(), None, None);
...
mod volume_profile_surface_tests {
    ...
    fn test_volume_profile_surface_basic() {
        let chain = create_test_chain_with_liquidity();
        let days = vec![pos!(5.0), pos!(15.0), pos!(30.0)];
...
mod edge_case_tests {
    ...
    fn test_high_open_interest_concentration() {
        ...
        let strikes_data = [
            (pos!(90.0), 100u64),
            (pos!(95.0), 500u64),
            (pos!(100.0), 100000u64), // Extreme concentration
            (pos!(105.0), 500u64),
            (pos!(110.0), 100u64),
        ];
