...
fn test_long_strangle_integration() -> Result<(), Box<dyn Error>> {
    ...
    assert_relative_eq!(strategy.profit_area().unwrap().to_f64().unwrap(), 0.2439, epsilon = 0.001);
    strategy.best_ratio(&option_chain, FindOptimalSide::Upper);
    assert_relative_eq!(strategy.profit_ratio().unwrap().to_f64().unwrap(), 0.0518, epsilon = 0.001);
