...
fn test_short_strangle_with_greeks_integration() -> Result<(), Box<dyn Error>> {
    ...
    assert_relative_eq!(strategy.profit_area().unwrap().to_f64().unwrap(), 9.8081, epsilon = 0.001);
    strategy.best_ratio(&option_chain, FindOptimalSide::Upper);
    assert_relative_eq!(strategy.profit_ratio().unwrap().to_f64().unwrap(), 47.4665, epsilon = 0.001);
