...
fn test_iron_condor_integration() -> Result<(), Box<dyn Error>> {
    ...
    assert_relative_eq!(strategy.profit_area().unwrap().to_f64().unwrap(), 19.5798, epsilon = 0.001);
    strategy.best_ratio(&option_chain, FindOptimalSide::Upper);
    assert_relative_eq!(strategy.profit_ratio().unwrap().to_f64().unwrap(), 237.3819, epsilon = 0.001);
