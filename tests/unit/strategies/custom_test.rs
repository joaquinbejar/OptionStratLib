...
mod tests {
    ...
    fn test_custom_strategy_get_option_basic_type() {
        ...
        let has_call = option_types
            .iter()
            .any(|opt| matches!(opt.side, Side::Long));
        let has_put = option_types
            .iter()
            .any(|opt| matches!(opt.side, Side::Short));
        assert!(has_call);
        assert!(has_put);
    }
    ...
    fn test_custom_strategy_get_positions() {
        ...
        assert_eq!(
            ...
            Positive::new(155.0).unwrap()
        );
        assert_eq!(
            positions[1].option.strike_price,
